...
func GetAdminMiddlewareFunc(jwtMaker *JWTMaker, handlerFunc http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		claims, err := verifyClaimsFromCookie(w, r, jwtMaker)
		if err != nil {
			http.Error(w, fmt.Sprintf("error verifying token: %v", err), http.StatusUnauthorized)
		...
		if !claims.IsAdmin {
			http.Error(w, fmt.Sprintf("user is not admin: %v", err), http.StatusUnauthorized)
			return
		}
		ctx := context.WithValue(r.Context(), AuthKey{}, claims)
		handlerFunc(w, r.WithContext(ctx))
